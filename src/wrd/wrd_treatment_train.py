...
if __name__ == "__main__":
    ...
    for x in m.fs.costing.component_objects([Var, Expression], descend_into=False):
        ...
        if "aggregate_flow_" in x.name and not x.is_indexed():
            ...
            try:
                ...
                print(f"\n{x.name}: {value(x)}")
                print(value(pyunits.convert(x, to_units=pyunits.gallon / pyunits.month)))
    ...
    m.fs.ammonium_sulfate_addition.unit.chemical_soln_flow_vol.fix(21990 * pyunits.gallon / pyunits.year)
    ...
    m.fs.sulfuric_acid_addition.unit.chemical_soln_flow_vol.fix(217166 * pyunits.gallon / pyunits.year)

    m.fs.sodium_hydroxide_addition.unit.dose.unfix()
    m.fs.sodium_hydroxide_addition.unit.chemical_soln_flow_mass.fix(14 * pyunits.ton / pyunits.month)
    ...
    m.fs.calcium_hydroxide_addition.unit.chemical_soln_flow_vol.fix(339108 * pyunits.gallon / pyunits.year)

    m.fs.sodium_bisulfite_addition.unit.dose.unfix()
    # m.fs.calcium_hydroxide_addition.unit.chemical_soln_flow_mass.fix(29 * pyunits.ton / pyunits.month)
    m.fs.sodium_bisulfite_addition.unit.chemical_soln_flow_vol.fix(39508 * pyunits.gallon / pyunits.year)

    solver = get_solver()
    results = solver.solve(m)
    assert_optimal_termination(results)

    for b in m.fs.component_objects(Block, ):
        ...
        x = pyunits.convert(b.ratio_in_solution, to_units=pyunits.gallon / pyunits.gallon)
        print(f"\t{'Ratio In Solution:':<30} {value(x):.2f} {pyunits.get_units(x)}")
        x = pyunits.convert(b.chemical_soln_flow_vol, to_units=pyunits.gallon / pyunits.month)
        print(f"\t{'Chemical Solution Vol Flow:':<30} {value(x):.2f} {pyunits.get_units(x)}")
        x = pyunits.convert(b.chemical_soln_flow_mass, to_units=pyunits.ton / pyunits.month)
        print(f"\t{'Chemical Solution Mass Flow:':<30} {value(x):.2f} {pyunits.get_units(x)}")
        ...
        x = pyunits.convert(m.fs.costing.aggregate_flow_costs[b.config.chemical], to_units=pyunits.USD_2021 / pyunits.month)
        ...
        try:
            ...
            print(f"\t{'Unit Cost ($/ton):':<30} {value(uc):.2f} {pyunits.get_units(uc)}")
        except:
            uc = pyunits.convert(uc, to_units=pyunits.USD_2021 / pyunits.gallon)
            print(f"\t{'Unit Cost ($/gal):':<30} {value(uc):.2f} {pyunits.get_units(uc)}")
