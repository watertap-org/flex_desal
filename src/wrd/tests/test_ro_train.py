...
@pytest.mark.unit
def test_ro_higher_salinity():
    # Input values from 3/13/21, but with double the feed salinity
    expected_power = 202.76 * pyunits.kW
    expected_product_flow = 1845.91 * pyunits.gal / pyunits.min
    expected_SEC = pyunits.convert(
        expected_power / expected_product_flow, to_units=pyunits.kWh / pyunits.m**3
    )
