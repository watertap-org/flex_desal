...
def test_wrd_treatment_train_8_19_21(num_pro_trains):
    ...
    assert pytest.approx(value(m.fs.disposal.unit.costing.variable_operating_cost), rel=0.35) == value(expected_brine_flow_cost)
    # Feedwater
    expected_feed_flow_cost = 241400 * pyunits.USD_2021 / pyunits.month
    assert pytest.approx(value(m.fs.feed.costing.variable_operating_cost), rel=0.35) == value(expected_feed_flow_cost)
...
def test_wrd_treatment_train_3_13_21(num_pro_trains):
    ...
    m = main(num_pro_trains=num_pro_trains,num_tsro_trains=3, uf_split_fraction=[0.4, 0.4, 0.2], file=file)    
