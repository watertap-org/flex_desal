...
def test_chem_addition_missing_data():
    ...
    with pytest.raises(ValueError, match=msg):
        _ = ca.build_system(chemical_name=None) # Why is this build system and not main?
...
def test_chem_addition_setting_dose():
    ...
    assert pytest.approx(value(pyunits.convert(m.fs.chem_addition.unit.chemical_soln_flow_vol, to_units=pyunits.gallon / pyunits.day)),rel=1e-3) == expected_chem_flow
    assert pytest.approx(value(pyunits.convert(m.fs.costing.aggregate_flow_costs['sodium_bisulfite'], to_units=pyunits.USD_2021 / pyunits.day)), rel=1e-3) == expected_chem_cost
