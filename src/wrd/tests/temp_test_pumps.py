...
def filter_pump_test_points(pump_data, pump_type='RO_feed'):
    ...
    pump_data = pump_data[pump_data['head'] <= head_limit(pump_data['flow']/1e3, pump_type=pump_type)*1e2]
    # Filter out points to the right of max flow at diff. speeds
    pump_data = pump_data[pump_data['head'] >= max_flows(pump_data['flow']/1e3, pump_type=pump_type)*1e2]
    # Filter out ????
