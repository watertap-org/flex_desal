...
def report_membrane_properties(m, solve_num):
    ...
    print(
        f"Calculated membrane porosity: {value(m.fs.ro.unit.feed_side.spacer_porosity)}"
    )
    print(
        f"Calculated water permeability (A): {value(m.fs.ro.unit.A_comp[0,'H2O'])} m/s/Pa"
    )
    print(
        f"Calculated salt permeability (B): {value(m.fs.ro.unit.B_comp[0,'NaCl'])} m/s"
    )
    print(
        f"Calculated pressure drop across membrane: {value(pyunits.convert(m.fs.ro.unit.deltaP[0], to_units=pyunits.psi))} psi"
    )
...
def calc_membrane_porosity(Qin, Qperm, Cin, Cperm, Pin, Pout):
    ...
    """
    m = solve_ro_module(Qin=Qin, Cin=Cin, Tin=298, Pin=Pin, Pout=Pout, stage_num=1)
...
if __name__ == "__main__":
    ...
    m = calc_membrane_porosity(
        Qin=Qin, Qperm=Qperm, Cin=Cin, Cperm=Cperm, Pin=Pin, Pout=Pout
    )
