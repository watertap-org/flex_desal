...
def build_srp(
    ...
    m.fs.power_gen_constr = Constraint(
        ...
        == pyunits.convert(
            ...
            * pyunits.convert(
                m.fs.cooling_tower.unit.to_evaporation_state[0].flow_vol_phase["Liq"],
...
def set_srp_operating_conditions(m):
    ...
    split_ab = 350 / (350 * 2 + 450)  # 0.30434782608695654
...
def add_bcs_basic(m):
    for bc_label in m.BCs:
        m.fs.add_component(bc_label, FlowsheetBlock(dynamic=False))
        bc_fs = m.fs.find_component(bc_label)
        build_separator(
            bc_fs,
            prop_package=m.fs.properties_feed,
...
def print_stream_flows(m, w=30):
    ...
    for bc_label in m.BCs:
        ...
        try:
            ...
            report_separator(bc_fs.unit, w=w)
